function New-AzureLabVM {
    ...
    if ($vmCount[0] -ne 0) {
        ...
        for ($i = 0; $i -lt $vmCount[0]; $i++) {
            $server = ("AZ-WSRV-" + $(Get-Random -Minimum 1000 -Maximum 10000) + ([char]($i + 97))).ToUpper()
    ...
    if ($vmCount[1] -ne 0) {
        ...
        for ($i = 1; $i -le $vmCount[1]; $i++) {
            $pc = ("AZ-WCLT-" + $(Get-Random -Minimum 1000 -Maximum 10000) + ([char]($i + 96))).ToUpper()
    ...
    if ($vmCount[2] -ne 0) {
        for ($i = 1; $i -le $vmCount[2]; $i++) {
            $vmName = ("AZ-LSRV-" + $(Get-Random -Minimum 1000 -Maximum 10000) + ([char]($i + 96))).ToUpper()
